...
def generate_automatic_mask(
    ...
    normalized_img = (one_band_mask - np.min(one_band_mask)) / (  # type: ignore
        np.max(one_band_mask) - np.min(one_band_mask)  # type: ignore
    )
    colored_img = plt.cm.tab20b(normalized_img)[:, :, :3]  # type: ignore # pylint: disable=E1101
...
def sam_prompt_bbox(
    ...
    if bboxes_geo is not None:
        ...
        bboxes_geo = predictor.transform.apply_boxes_torch(bboxes_geo, img.shape[:2])  # type: ignore
    if points_geo is not None:
        ...
        points_geo = predictor.transform.apply_coords_torch(points_geo, img.shape[:2])  # type: ignore
